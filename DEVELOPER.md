...
>
> **Document status** — **v0.1 · 2026‑06‑23.** Sources: the live source `ALETHEIA.html` (single file). Coverage: all subsystems, functions, IDs, storage keys and export semantics were read from source, not memory. Feature set: ALETHEIA typographic identity, 21 study shapes/icons with centred labels, area eraser (3 sizes + ring cursor), distinct tool cursors, unified six-colour note/highlighter palette, study slash‑blocks, links open in new window, Reflection panel (Markdown in/out + clear + AI + **proportional canvas scaling**), Mac ⌘ shortcuts, multi-select format (`selTargets()`), system clipboard paste, filename-preserving export, reliable second-file loading. Arrange subsystem (group/order/align/distribute) fully removed from code, UI and keyboard. Confidence: high; line numbers are intentionally omitted (they drift) — navigate by function names and section banners (`// === ... ===`).
...
**Shipped in v0.1**: modes (Browse/Edit); select (click/ctrl/marquee); move/resize/rotate (parent‑coord pivot, live handles); multi-select format via `selTargets()`; insert (text / 21 study shapes+icons with centred labels / image / table / coloured notes / links); table editing; text editing + slash blocks (basic + study); pencil/highlighter/area‑eraser with distinct cursors, unified palette; hyperlinks that open in a new window; docked Reflection panel + AI + Markdown import/export + clear + **proportional canvas scaling**; system clipboard paste; filename-preserving export (`<name>-editado.html`); reliable second-file loading; undo/redo; grid; ALETHEIA branding (inline favicon); Mac ⌘ shortcuts.
